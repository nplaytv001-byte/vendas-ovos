...
# --- CLASSE PDF (Ajustada para fpdf2) ---
class PDF(FPDF):
    def header(self):
...
def gerar_pdf(df_vendas):
    ...
    for _, row in df_vendas.iterrows():
        ...
        pago = (row['pago_dinheiro'] or 0) + (row['pago_pix'] or 0)
        
        if row['pendente'] <= 0:
            pdf.set_fill_color(144, 238, 144) # Verde
            status = "PAGO"
        elif data_venda < hoje:
            pdf.set_fill_color(255, 182, 193) # Vermelho
            status = "ATRASADO"
        else:
        ...
        pdf.cell(55, 10, str(row['nome'])[:28], 1)
        pdf.cell(25, 10, f"{row['total_nota']:.2f}", 1)
        pdf.cell(25, 10, f"{pago:.2f}", 1)
        pdf.cell(25, 10, f"{row['pendente']:.2f}", 1)
        pdf.cell(35, 10, status, 1, 1, 'C', True)

    # Retorna o PDF como bytes (essencial para o Streamlit Cloud)
    return pdf.output()
...
if aba == "Início":
    ...
    if not clientes_df.empty:
        with st.form("venda"):
            ...
            # ORDEM ALTERADA: Branco na frente do Vermelho
            ...
            st.write(f"**Total: R$ {v_unit * qtd:.2f}**")
            ...
            if st.form_submit_button("Finalizar"):
                ...
                conn.execute('''INSERT INTO vendas (cliente_id, data, produto, valor_unit, qtd, total_nota, pago_dinheiro, pago_pix, pendente)
                                VALUES (?,?,?,?,?,?,?,?,?)''', (c_id, date.today(), f"{cor} {tipo}", v_unit, qtd, total, p_din, p_pix, pend))
                conn.commit()
